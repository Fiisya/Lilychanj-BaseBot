...
└───···`)
await delay(1999);
  Risa.sendMessage('120363207338528857' + "@g.us", { text: `*_Connected!_* 🕊️
...
alfixdWlcm = await getBuffer(ppuser)
alfixdLft = await getBuffer(ppuser)
                if (anu.action == 'add') {
                const alfixdbuffer = await getBuffer(ppuser)
                let alfixdName = num
                const xtime = moment.tz('Asia/Kolkata').format('HH:mm:ss')
	            const xdate = moment.tz('Asia/Kolkata').format('DD/MM/YYYY')
	            const xmembers = metadata.participants.length
                alfixdbody = Alfitext(`
- Hello @${alfixdName.split("@")[0]} 👋 Welcome To Group  ${metadata.subject} 
-  ${xmembers}th Member
-  Joined At ${xtime} On ${xdate}
...
renderLargerThumbnail: true
                	...
                	const alfixdbuffer = await getBuffer(ppuser)
                    const alfixdtime = moment.tz('Asia/Kolkata').format('HH:mm:ss')
	                const alfixddate = moment.tz('Asia/Kolkata').format('DD/MM/YYYY')
                	let alfixdName = num
                    const alfixdmembers = metadata.participants.length
                    alfixdbody = Alfitext(`
- Goodbye @${alfixdName.split("@")[0]} 👋
- ${alfixdmembers}th Member
- Out At ${alfixdtime} On ${alfixddate}`)
          Risa.sendMessage(anu.id,
 { text: alfixdbody,
 contextInfo:{
 mentionedJid:[num],
...
renderLargerThumbnail: true
        	...
        	let alfixdName = num
          let a = `Congratulations @${alfixdName.split("@")[0]}, on being promoted to admin of this group ${metadata.subject} 👤`
          Risa.sendMessage(anu.id, {
            text: a,
            contextInfo: {
              externalAdReply: {
        	...
        	let alfixdName = num
          let a = `@${alfixdName.split("@")[0]}, on being demote to member of this group ${metadata.subject}`
          Risa.sendMessage(anu.id, {
            text: a,
            contextInfo: {
              externalAdReply: {
