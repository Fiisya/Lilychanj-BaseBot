...
await Risa.sendMessage(m.chat, { react: { text: "⏱️",key: m.key,}})   
try{
let anu = await fetchJson(`https://aemt.uk.to/download/fbdl?url=${text}`)
...
case 'chatgpt': {
 ...
 let hiasan = "`"
 let api = await fetchJson(`https://aemt.uk.to/prompt/gpt?prompt=${logic}&text=${text}`) 
 let ress = api.result
...
let cekurukuk = await fetchJson(`https://aemt.uk.to/prompt/gpt?prompt=${prompt}&text=${text}`)
...
case 'lirik': {
 ...
 reply3(mess.wait)
 try {
 let lirik = await (await fetch(`https://aemt.uk.to/lirik?text=${text}`)).json()
...
case 'gpt4': {
 ...
 reply3(mess.wait)
 try {
 let gpt4 = await (await fetch(`https://aemt.uk.to/gpt4?text=${text}`)).json()
...
case 'gptgo': {
 ...
 reply3(mess.wait)
 try {
 let gptgo = await (await fetch(`https://aemt.uk.to/gptgo?text=${text}`)).json()
...
await Risa.sendMessage(m.chat, { react: { text: "🌚",key: m.key,}}) 
 ...
 const data = await fetchJson(`https://aemt.uk.to/bardimg?url=${anuu}&text=${encodeURIComponent(text)}`)
...
case 'ai':{
  ...
  reply3(`Maaf Ya Terpaksa Aku Tendang 😖, Perintah Admin Sih..`);
      ...
      await Risa.sendMessage(m.chat, { react: { text: "💬", key: m.key } });        
        let aii = await fetchJson(`https://aemt.uk.to/prompt/gpt?prompt=${logic}&text=${text}`);
      reply3(aii.result);
  }
};
break 
...
case 'gemini': {
 ...
 reply3(mess.wait)
 try {
 let gemini = await (await fetch(`https://aemt.uk.to/gemini?text=${text}`)).json()
...
case 'storyanime': {
 let sni = await (await fetch('https://aemt.uk.to/download/storyanime')).json()
 let sni2 = sni.result
...
case 'gptturbo': {
 ...
 reply2(mess.wait)
 try {
 let gptturbo = await (await fetch(`https://aemt.uk.to/v2/turbo?text=${text}`)).json()
...
case 'tiktoksearch': case 'tiktoks': case 'ttsearch':{
  if (!text) return warning(`Gunakan dengan cara .tiktoksearch *query*\n\n_Contoh_\n\n${prefix+command} jj epep`)
  await Risa.sendMessage(m.chat, { react: { text: "⏱️",key: m.key,}})
  try{
...
`
 ...
 mediaType: 1,
 previewType: 1,
 body: "ɪ ᴀᴍ ᴀɴ ᴀᴜᴛᴏᴍᴀᴛᴇᴅ ꜱʏꜱᴛᴇᴍ (ᴡʜᴀᴛꜱᴀᴘᴘ ʙᴏᴛ) ᴛʜᴀᴛ ᴄᴀɴ ʜᴇʟᴘ ᴛᴏ ᴅᴏ ꜱᴏᴍᴇᴛʜɪɴɢ, ꜱᴇᴀʀᴄʜ ᴀɴᴅ ɢᴇᴛ ᴅᴀᴛᴀ / ɪɴꜰᴏʀᴍᴀᴛɪᴏɴ ᴏɴʟʏ ᴛʜʀᴏᴜɢʜ ᴡʜᴀᴛꜱᴀᴘᴘ.",
 thumbnailUrl: menugambir,
 renderLargerThumbnail: true,
...
let result = await fetchJson(`https://aemt.uk.to/` + command + `?text=${text}`)
...
case 'capcut': case 'facebook': case 'fb': case 'fbdl': case 'tt3': case 'ig2': case 'aio': {
 ...
 try {
 const axios = require('axios');
 const apiUrl = `https://aemt.uk.to/download/aio?url=${encodeURIComponent(text)}`;
...
case 'thread': {
 ...
 const axios = require('axios');
 const threadAPI = `https://aemt.uk.to/download/threads?url=${encodeURIComponent(text)}`;
...
case 'twtdl': {
 ...
 const axios = require('axios');
 const apiUrl = `https://aemt.uk.to/download/twtdl?url=${encodeURIComponent(text)}`;
...
case 'music': {
if (!text) return reply3(`Contoh: ${prefix + command} sephia`);
reply3(mess.wait);
try {
...
if (!convert) return reply3('Audio Tidak Ditemukan');
if (convert.seconds >= 3600) {
return reply3('Audio is longer than 1 hour!');
}
let audioUrl;
try {
