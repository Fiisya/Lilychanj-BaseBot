...
if (db.data.chats[m.chat].autodown) {
    try {
        ...
        if (budy.match(/instagram\.com/)) {
            ...
            await Risa.sendMessage(m.chat, { video: { url: anu.result.url }, caption: "Auto Download ✅" }, { quoted: m });
            await Risa.sendMessage(m.chat, { react: { text: "☑️", key: m.key } });
        } else if (budy.match(/tiktok\.com/)) {
            await Risa.sendMessage(m.chat, { react: { text: "⏱️", key: m.key } });
            let anu = await fetchJson(`https://btch.us.kg/download/v2/ttdl?url=${budy}`);
            await Risa.sendMessage(m.chat, { video: { url: anu.result.video }, caption: "Auto Download ✅" }, { quoted: m });
            await Risa.sendMessage(m.chat, { react: { text: "☑️", key: m.key } });
        } else if (budy.match(/facebook\.com/)) {
            await Risa.sendMessage(m.chat, { react: { text: "⏱️", key: m.key } });
            let anu = await fetchJson(`https://btch.us.kg/download/fbdl?url=${budy}`);
            await Risa.sendMessage(m.chat, { video: { url: anu.result.Normal_video }, caption: "Auto Download ✅" }, { quoted: m });
            await Risa.sendMessage(m.chat, { react: { text: "☑️", key: m.key } });
        } else if (budy.match(/youtube\.com|youtu\.be/)) {
            await Risa.sendMessage(m.chat, { react: { text: "⏱️", key: m.key } });
            let anu = await fetchJson(`https://btch.us.kg/download/ytdl?url=${budy}`);
            await Risa.sendMessage(m.chat, { video: { url: anu.result.mp4 }, caption: "Auto Download ✅" }, { quoted: m });
            await Risa.sendMessage(m.chat, { react: { text: "☑️", key: m.key } });
        }
    } catch (err) {
        await Risa.sendMessage(m.chat, { react: { text: "✖️", key: m.key } });
...
async function EncryptJs(code) {
    let res = await fetch(`https://api.alyachan.dev/api/obfuscator?code=${code}&apikey=f12QOb`);
...
async function EncryptJs2(code) {
    let res = await fetch(`https://ai.xterm.codes/api/tools/js-protector?code=${code}&key=Bell409`);
    let jsonResponse = await res.json(); // Mendapatkan respons JSON
...
case 'autodown': {
 if (args[0] === "on") {
 if (db.data.chats[m.chat].autodown) return m.reply(`Sudah Aktif Sebelumnya 🚩`)
 db.data.chats[m.chat].autodown = true
 m.reply(`Auto Download Aktif 🚩\n\n- TikTok\n- YouTube\n- Instagram\n- Facebook`)
 } else if (args[0] === "off") {
 if (!db.data.chats[m.chat].autodown) return m.reply(`Sudah Nonaktif Sebelumnya 🕊`)
 db.data.chats[m.chat].autodown = false
 m.reply(`Auto Download Nonaktif 🚩`)
 } else {
